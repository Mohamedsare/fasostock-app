...
class SalesRepository {
  SalesRepository([SupabaseClient? client]) : _client = client ?? Supabase.instance.client;
  ...
  Future<List<Sale>> list(
    ...
    var q = _client.from('sales').select(_saleSelect).eq('company_id', companyId);
    ...
    return (data as List).map((e) => Sale.fromJson(Map<String, dynamic>.from(e as Map))).toList();
  }

  Future<Sale?> get(String id) async {
    final data = await _client.from('sales').select(_saleSelect).eq('id', id).maybeSingle();
  ...
  Future<List<SaleItem>> getItems(String saleId) async {
    final data = await _client
        .from('sale_items')
        .select('id, sale_id, product_id, quantity, unit_price, discount, total, product:products(id, name, sku, unit)')
        .eq('sale_id', saleId)
        .order('created_at');
    return (data as List).map((e) => SaleItem.fromJson(Map<String, dynamic>.from(e as Map))).toList();
  }

  Future<List<SalePayment>> getPayments(String saleId) async {
    final data = await _client.from('sale_payments').select('id, sale_id, method, amount, reference').eq('sale_id', saleId);
    return (data as List).map((e) => SalePayment.fromJson(Map<String, dynamic>.from(e as Map))).toList();
  }
  ...
  Future<Sale> create(CreateSaleInput input, String userId) async {
    ...
    try {
      ...
      if (msg.contains('create_sale_with_stock') && msg.contains('PGRST202') && msg.contains('p_client_request_id')) {
        final payload2 = Map<String, dynamic>.from(payload)..remove('p_client_request_id');
