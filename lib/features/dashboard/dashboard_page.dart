...
class _DashboardPageState extends ConsumerState<DashboardPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (!mounted) return;
      await _loadDataFromOffline();
      if (!mounted) return;
      // Sync en arrière-plan : les KPI locaux sont déjà affichés ; pas de blocage sur le réseau.
      unawaited(_runSyncThenRefresh());
    });
  }
  ...
  Future<void> _runSyncThenRefresh() async {
    ...
    try {
      ...
      if (mounted && result.errors > 0) {
        AppToast.error(
          context,
          'Certaines données n\'ont pas pu être synchronisées. Réessayez plus tard.',
        );
      }
    } catch (e) {
      if (mounted) {
        AppErrorHandler.show(
          ...
          e,
          fallback:
              'Synchronisation échouée. Les données locales restent affichées.',
        );
      }
    }
  ...
  Future<void> _loadDataFromOffline({bool silent = false}) async {
    ...
    try {
      ...
      if (mounted) {
        final message = AppErrorHandler.toUserMessage(
          e,
          fallback: 'Impossible de charger le tableau de bord. Réessayez.',
        );
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    if (permissions.hasLoaded &&
        !permissions.hasPermission(Permissions.dashboardView)) {
      final requiredLabel =
          Permissions.labels[Permissions.dashboardView] ??
            ...
            children: [
              ...
              if (_error != null)
                ...
                _buildChartAndSidebar(
                  context,
                  isWide,
                  context.watch<PermissionsProvider>().hasPermission(
                    Permissions.salesCreate,
                  ),
                  context.watch<PermissionsProvider>().hasPermission(
                    Permissions.salesInvoiceA4,
                  ),
                ),
                SizedBox(height: isWide ? 20 : 12),
              ...
              children: [
                FilterChip(
                  ...
                  label: Text(
                    'Entreprise',
                    style: TextStyle(
                      color: _scope == 'company'
                          ? theme.colorScheme.primary
                          : theme.colorScheme.onSurface,
                      fontWeight: _scope == 'company'
                          ? FontWeight.w600
                          : FontWeight.w500,
                    ),
                  ),
                  ...
                  if (_scope == 'store' && company.stores.length > 1)
                    ConstrainedBox(
                      ...
                      child: DropdownButtonFormField<String>(
                        initialValue:
                            company.currentStoreId != null &&
                                company.stores.any(
                                  (s) => s.id == company.currentStoreId,
                                )
        ...
        children: [
          Padding(
            padding: EdgeInsets.fromLTRB(
              MediaQuery.sizeOf(context).width >= 600 ? 20 : 12,
              12,
              MediaQuery.sizeOf(context).width >= 600 ? 20 : 12,
              8,
            ),
                ...
                return GridView.count(
                  ...
                  children: [
                    _dayStat(
                      theme,
                      'CA du jour',
                      formatCurrency(d.daySalesSummary.totalAmount),
                    ),
                    _dayStat(theme, 'Ventes', '${d.daySalesSummary.count}'),
                    _dayStat(
                      theme,
                      'Articles vendus',
                      '${d.daySalesSummary.itemsSold}',
                    ),
                    _dayStat(
                      theme,
                      'Marge du jour',
                      formatCurrency(d.daySalesSummary.margin),
                      isHighlight: true,
                    ),
                    _dayStat(
                      theme,
                      'Achats du jour',
                      '${formatCurrency(d.dayPurchasesSummary.totalAmount)}\n${d.dayPurchasesSummary.count} commande(s)',
                    ),
                  ],
                );
              },
  ...
  Widget _buildKpiGrid(BuildContext context) {
    ...
    final marginRate = d.salesSummary.totalAmount > 0
        ? ((d.salesSummary.margin / d.salesSummary.totalAmount) * 100)
              .toStringAsFixed(1)
        : '0';
    ...
    final kpis = [
      _KpiDef(
        'Chiffre d\'affaires',
        formatCurrency(d.salesSummary.totalAmount),
      ...
      _KpiDef(
        'Ventes',
        '${d.salesSummary.count}',
        Icons.shopping_cart_rounded,
        const Color(0xFF059669),
      ),
      _KpiDef(
        'Ticket moyen',
        formatCurrency(d.ticketAverage),
        Icons.receipt_long_rounded,
        const Color(0xFF0EA5E9),
      ),
      _KpiDef(
        'Produits vendus',
        '${d.salesSummary.itemsSold}',
        Icons.inventory_2_rounded,
        const Color(0xFF2563EB),
      ),
      _KpiDef(
        'Marge',
        formatCurrency(d.salesSummary.margin),
      ...
      _KpiDef(
        'Achats',
        formatCurrency(d.purchasesSummary.totalAmount),
      ...
      _KpiDef(
        'Valeur stock',
        formatCurrency(d.stockValue.totalValue),
      ...
      _KpiDef(
        ...
        const Color(0xFFD97706),
        linkToInventory: d.lowStockCount > 0,
      ),
    ];
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final kpiAspectRatio = w < 400
            ? 1.05
            : (w < 600 ? 1.15 : (w < 900 ? 1.25 : 1.35));
                              ...
                              children: [
                                Text(
                                  ...
                                  style: theme.textTheme.titleMedium?.copyWith(
                                    fontWeight: FontWeight.w700,
                                    color: k.color == const Color(0xFF059669)
                                        ? k.color
                                        : theme.colorScheme.onSurface,
                                  ),
                                  maxLines: 1,
  ...
  Widget _buildChartAndSidebar(
    BuildContext context,
    bool isWide,
    bool canOpenPosQuick,
    bool canOpenInvoiceA4,
  ) {
    final theme = Theme.of(context);
    final d = _data;
    if (d == null) return const SizedBox.shrink();
                ...
                child: BarChart(
                  BarChartData(
                          ...
                          (e) => BarChartGroupData(
                            ...
                            barRods: [
                              BarChartRodData(
                                ...
                                width: (d.salesByDay.length > 14 ? 6 : 12)
                                    .toDouble(),
                    ...
                    titlesData: FlTitlesData(
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          showTitles: true,
                          reservedSize: 36,
                          interval: (d.salesByDay.length / 8)
                              .clamp(1, 31)
                              .toDouble(),
                    ...
                    barTouchData: BarTouchData(
                      // fl_chart : bug tooltip / TextPainter si une seule barre — interactions off.
                      enabled: d.salesByDay.length > 1,
  ...
  Widget _buildFooter(BuildContext context, ({String from, String to}) range) {
    ...
    final fromStr = fromParsed != null
        ? _formatShortDate(fromParsed)
        : range.from;
