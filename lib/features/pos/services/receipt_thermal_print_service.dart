...
class ReceiptThermalPrintService {
  ...
  static Future<Uint8List> _buildPdf(ReceiptTicketData data) async {
    ...
    final fontStoreTitle = pw.Font.ttf(await rootBundle.load('assets/fonts/ArchivoBlack-Regular.ttf'));
    ...
    final bold = pw.TextStyle(font: fontBold, fontSize: 8, color: PdfColors.black);
    ...
    final totalBig = pw.TextStyle(font: fontBold, fontSize: 11, color: PdfColors.black);
    final tiny = pw.TextStyle(font: font, fontSize: 7.5, color: PdfColors.black);
    final metaCourier = pw.TextStyle(font: pw.Font.courier(), fontSize: 8.5, color: PdfColors.black);

    pw.Widget hrLong() => pw.Text(ReceiptTicketLayout.sepLong, style: tiny, textAlign: pw.TextAlign.center);
    pw.Widget hrMid() => pw.Text(ReceiptTicketLayout.sepMid, style: tiny, textAlign: pw.TextAlign.center);
    ...
    final children = <pw.Widget>[
      ...
      if (data.storeAddress != null && data.storeAddress!.trim().isNotEmpty)
        pw.Padding(
          ...
          child: pw.Text(_sanitizeForPdf(data.storeAddress!.trim()), style: base, textAlign: pw.TextAlign.center),
        ),
      if (tel.isNotEmpty)
        pw.Padding(
          padding: const pw.EdgeInsets.only(top: 2),
          child: pw.Text(_sanitizeForPdf(tel), style: base, textAlign: pw.TextAlign.center),
        ),
      pw.SizedBox(height: 6),
      pw.Center(
        child: pw.Text(
          _sanitizeForPdf(ReceiptTicketLayout.metaFactureDateHeureLine(saleNumber, data.date)),
          style: metaCourier,
            ...
            pw.TableRow(
              children: [
                pw.Padding(
                  ...
                  child: pw.Text(_truncate(_sanitizeForPdf(item.name), 22), style: base),
                ),
                pw.Text('${item.quantity}', style: base, textAlign: pw.TextAlign.center),
                pw.Text('${item.unitPrice.round()}', style: base, textAlign: pw.TextAlign.right),
                pw.Text('${item.total.round()}', style: base, textAlign: pw.TextAlign.right),
              ],
            ),
        ],
      ),
      ...
      pw.SizedBox(height: 8),
      pw.Text('Merci pour votre achat !', style: bold, textAlign: pw.TextAlign.center),
      pw.SizedBox(height: 6),
      hrMid(),
      pw.Text('Powered by FasoStock POS', style: tiny, textAlign: pw.TextAlign.center),
      hrMid(),
    ];
